 ...
 */
package com.oracle.saga.cloudbank.banka.stubs;
...
public class AccountsService {
    ...
    public static final String SPAN_BANK_A = "bankA";
            ...
            if (statusInsert != -1 ) {
                ...
                if(bal!=-1.0){
                    ...
                    updateOperationStatus(sagaId,ONGOING,accountsJsonObj.get(TRANSACTION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""), 
                            accountsJsonObj.get(OPERATION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                }else{
                ...
                if (validateUserWithdrawing(accountsJsonObj)) {
                    ...
                    if (futBal > -1.0) {
                        ...
                        updateOperationStatus(sagaId, COMPLETED,
                                accountsJsonObj.get(TRANSACTION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""), 
                                accountsJsonObj.get(OPERATION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                    } else {
                        updateOperationStatus(sagaId, COMPLETED,
                                accountsJsonObj.get(TRANSACTION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""), 
        ...
        try (OracleJsonParser parser = jsonFactory.createJsonTextParser(inputReader)) {
            ...
            if(statusInsert1 != -1  || statusInsert2 != -1){
                if(statusInsert1==-1 ){
                    ...
                    accountsJsonObjDeposit.get(OPERATION).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                }
                if(statusInsert2==-1 ){
    ...
    public double chktxn(OracleJsonObject accountsJsonObj) {
        ...
        finalBal = finalBal - Double.parseDouble(accountsJsonObj.get(AMOUNT).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
    ...
    public void updateOperationStatus(String sagaId, String state, String txnType, String operation){
        String updateOperationStateBooking = "UPDATE bankA_book set operation_status = ? where SAGA_ID = ? and transactionType = ? and operationType = ?";
        try (PreparedStatement stmt = connection.prepareStatement(updateOperationStateBooking)) {
            stmt.setString(1, state);
            stmt.setString(2, sagaId);
    ...
    /**
     * fetchRecentBankALogs helps select top 10 rows from bankA_book table.
     */
    public static List<BankALogDTO> fetchRecentBankALogs(Connection connection, String ucid, List<String> acctNums) {
        ...
        List<BankALogDTO> results = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
            ...
            logger.error("UNABLE TO SELECT logs FROM Bank A table");
        }
        return results;
    }
