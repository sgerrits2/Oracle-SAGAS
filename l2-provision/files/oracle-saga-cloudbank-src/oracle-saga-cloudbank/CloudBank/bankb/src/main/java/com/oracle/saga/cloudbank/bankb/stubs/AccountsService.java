 ...
 */
package com.oracle.saga.cloudbank.bankb.stubs;
...
public class AccountsService {
    ...
    public static final String SPAN_BANK_B = "bankB";
            ...
            if (statusInsert != -1 ) {
                ...
                if(balanceAmount!=-1.0){
                    ...
                    updateOperationStatus(sagaId,FAILED,accountsJsonObj.get(TRANSACTIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""),
                            accountsJsonObj.get(OPERATIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                }
     ...
     */
    public String withdrawMoneyCheck(String accountPayload, String sagaId) {
        ...
        try (OracleJsonParser parser = jsonFactory.createJsonTextParser(inputReader)) {
            ...
            if (statusInsert != -1) {
                if (validateUserWithdrawing(accountsJsonObj)) {
                    ...
                    futBal = chktxn(accountsJsonObj);

                    if (futBal > -1.0) {
                        final CompensationData accountsCompensationInfo = new CompensationData();
                        accountsCompensationInfo.setSagaId(sagaId);
                        accountsCompensationInfo
                                .setUcid(accountsJsonObj.get("ucid").toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                        accountsCompensationInfo.setOperationType(
                                accountsJsonObj.get(OPERATIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                        ArrayList<CompensationData> temp = new ArrayList<>();
                        temp.add(accountsCompensationInfo);
                        accountsCompensationCache.put(sagaId, temp);
                        updateOperationStatus(sagaId, COMPLETED,
                                accountsJsonObj.get(TRANSACTIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""),
                                accountsJsonObj.get(OPERATIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                    } else {
                        updateOperationStatus(sagaId, COMPLETED,
                                accountsJsonObj.get(TRANSACTIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""),
        ...
        return String.valueOf(futBal);
    }

    /**
     * chktxn executes queries to perform balance checks
     * withdraw functionality.
     */
    public double chktxn(OracleJsonObject accountsJsonObj) {
        double finalBal;

        finalBal = fetchBalance(false, accountsJsonObj);
        finalBal = finalBal
                - Double.parseDouble(accountsJsonObj.get(AMOUNT).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
        ...
        try (OracleJsonParser parser = jsonFactory.createJsonTextParser(inputReader)) {
            ...
            if (statusInsert1 != -1  && statusInsert2 != -1) {
                ...
                if(balanceAmountDeposit!=-1.0){
                    ...
                    accountsJsonObjDeposit.get(OPERATIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                }else{
                ...
                if(validateUserWithdrawing(accountsJsonObjWithdraw)){
                    ...
                    if(balanceAmountWithdraw!=-1.0){
                        ...
                        accountsJsonObjDeposit
                                        .get(OPERATIONTYPE).toString().replaceAll(REG_EXP_REMOVE_QUOTES, ""));
                    }else{
     ...
     */
    public void updateOperationStatus(String sagaId, String state, String txnType, String operation){
        String updateOperationStateBooking = "UPDATE bankB_book set operation_status = ? where SAGA_ID = ? and transactionType = ? and operationType = ?";
        try (PreparedStatement stmt = connection.prepareStatement(updateOperationStateBooking)) {
            stmt.setString(1, state);
            stmt.setString(2, sagaId);
    ...
    /**
     * fetchRecentBankBLogs helps select top 10 rows from bankA_book table.
     */
    public static List<BankBLogDTO> fetchRecentBankBLogs(Connection connection, String ucid, List<String> acctNums) {
        ...
        if (acctNums != null && !acctNums.isEmpty()) {
            String placeholders = acctNums.stream()
                    .map(s -> "?")
                    .collect(Collectors.joining(", "));
            sql.append(" OR account_number IN (")
                    .append(placeholders)
                    .append(")");
        }
        sql.append(" ORDER BY created_at DESC");

        List<BankBLogDTO> results = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {

            int idx = 1;
            ps.setString(idx++, ucid);

            if (acctNums != null && !acctNums.isEmpty()) {
                for (String acct : acctNums) {
                    ps.setString(idx++, acct);
                }
            }
            ...
            while (rs.next()) {
                BankBLogDTO dto = new BankBLogDTO();
                dto.setLogId(rs.getLong("log_id"));
                dto.setSagaId(rs.getString("saga_id"));
                dto.setUcid(rs.getString("ucid"));
                dto.setOperationType(rs.getString(AccountsService.OPERATIONTYPE));
                ...
                dto.setOperationStatus(rs.getString("operation_status"));
            ...
            logger.error("UNABLE TO SELECT logs FROM Bank B table");
        }
        return results;
    }
